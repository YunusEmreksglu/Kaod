...
namespace MangaKB.Classlar.JsonClass
{
    public class AssetJson(string Konum)
    {
        ...
        private string SaveImage(FileInfo ImagePath, Image Image)
        {
        ...
        public AssetData Save(List<Box> Kutular, int ImageWidth, int ImageHeight, FileInfo ImagePath, Image Image)
        {
            ...
            Asset asset = new Asset()
            {
                ...
                name = ImagePath.Name,
                path = $"file:{SaveImage(ImagePath, Image)}",
                size = size,
                state = region.Count,
                type = 1
            };
